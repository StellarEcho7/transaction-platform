...
Task: Read the conversation history in this chat and save it to a file to `docs/spec/{NUMBER}-{FEATURE-NAME}/SPEC.md`.

Output format:

1. Determine the next sequential number for the feature folder (check existing folders in `docs/spec/` first, default to `001` if no files exist)
2. Parse the user's input to extract a short, descriptive feature name in kebab-case
3. Create the file at `docs/spec/{NUMBER}-{FEATURE-NAME}/SPEC.md`
